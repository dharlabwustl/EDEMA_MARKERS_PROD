            ...
            if img_idx>0 and img_idx < numpy_image.shape[2] and  filename_gray_data_np_copy.shape==csf_seg_np.shape:
                ...
                if os.path.exists(this_npyfile):
                    ...
                    if np.sum(numpy_image_mask[:,:,img_idx]) >0:
                        slice_3_layer1 = cv2.putText(slice_3_layer1,str(slice_number)+' PHE' , org, font,  fontScale, color, thickness, cv2.LINE_AA)
        ...
        for img_idx in range(numpy_image.shape[2]):
            ...
            if img_idx>0 and img_idx < numpy_image.shape[2] and  filename_gray_data_np_copy.shape==csf_seg_np.shape:
                ...
                if os.path.exists(this_npyfile):
                    ...
                    thickness = 2
                    if np.sum(numpy_image_mask[:,:,img_idx]) >0:
                        slice_3_layer1 = cv2.putText(slice_3_layer1,str(slice_number)+' ICH' , org, font,  fontScale, color, thickness, cv2.LINE_AA)
