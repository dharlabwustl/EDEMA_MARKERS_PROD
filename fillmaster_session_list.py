...
def append_sessionxmlinfo_to_analytics(args):
    try:
        ...
        columnvalue=""
        # try:
        columnvalue_1=subj_listfile_df[subj_listfile_df['ID'].str==str(xmlfile_dict['xnat:CTSession']['xnat:subject_ID'])].reset_index()
        # if len(columnvalue_1) >0 :
        subprocess.call("echo " + "I PASSED AT ::{}::{}  >> /workingoutput/error.txt".format(inspect.stack()[0][3],columnvalue_1) ,shell=True )
...
def append_dicominfo_to_analytics(session_id,scan_id,csvfilename,dir_to_save="./"):
    try:
        ...
        res_x,res_y,res_z,scanner_model,scanner_manufacturer,dateandtime,bodypart=get_dicom_information(df_scan.at[0,"URI"],dir_to_save)
...
def get_dicom_information(dicom_url,dir_to_save="./"):
    ...
    return res_x,res_y,res_z,scanner_model,scanner_manufacturer,dateandtime,bodypart
