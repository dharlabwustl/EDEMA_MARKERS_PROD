...
while IFS=',' read -ra array; do
  ...
  while IFS=',' read -ra array1; do
    ...
    call_insert_one_col_with_colname_arguments=('call_insert_one_col_with_colname' ${csvfilename} ${csvfilename} "FILENAME" ${grayscale_filename_basename_noext}) # ${csvfilename} ${csvfilename} ${output_directory}/$(basename ${mask_filename3%.nii*})_RATIO.csv )
    outputfiles_present=$(python3 dividemasks_into_left_right.py "${call_insert_one_col_with_colname_arguments[@]}")
