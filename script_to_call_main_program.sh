...
if [[ ${TYPE_OF_PROGRAM} == 'TEST_CODE_ON_SNIPR' ]]; then
#  /software/single_data_mask_on_gray_pdf.sh $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input1 /output
  /software/run_ssh_test.sh  $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input1 /output
fi
...
#    /software/dicom2nifti_call_20260206.sh  $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input1 /output
      /software/change_scan_type_to_z-axial.sh  $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input1 /output
...
if [[ ${TYPE_OF_PROGRAM} == 'TRANFORM_BEFORE_DEEPREG_COLIHM62_MOVING_LOCAL_COMPUTER' ]]; then
  echo " I AM AT TRANFORM_BEFORE_DEEPREG_COLIHM62_MOVING_LOCAL_COMPUTER" >> /software/ERROR.txt
  echo " I AM AT TRANFORM_BEFORE_DEEPREG_COLIHM62_MOVING_LOCAL_COMPUTER"
  /software/lin_transform_before_deepreg_COLIHM62_as_moving_local_computer.sh $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input1 /output
fi
...
if [[ ${TYPE_OF_PROGRAM} == 'AFFINE_TRAN_BEFORE_NONLIN_COLIHM62_MOVING' ]]; then
...
echo "$SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST "
if [[ ${TYPE_OF_PROGRAM} == 'APPLY_MAT_TRANFORM_BEFORE_DEEPREG_COLIHM62_MOVING_LOCAL_COMPUTER' ]]; then
  echo " I AM AT APPLY_MAT_TRANFORM_BEFORE_DEEPREG_COLIHM62_MOVING_LOCAL_COMPUTER" >> /software/ERROR.txt
  echo " I AM AT APPLY_MAT_TRANFORM_BEFORE_DEEPREG_COLIHM62_MOVING_LOCAL_COMPUTER"
  /software/lin_transform_before_deepreg_COLIHM62_as_moving_mat_on_mask_local_computer.sh $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input1 /output
fi
...
if [[ ${TYPE_OF_PROGRAM} == 'APPLY_MAT_ONLY_BEFORE_DEEPREG_COLIHM62_MOVING_RIS_SNIPR_NWU_COMP' ]]; then #
  echo " I AM AT APPLY_MAT_ONLY_BEFORE_DEEPREG_COLIHM62_MOVING_RIS_SNIPR_NWU_COMP" >> /software/ERROR.txt
  echo " I AM AT APPLY_MAT_ONLY_BEFORE_DEEPREG_COLIHM62_MOVING_RIS_SNIPR_NWU_COMP"
  /software/lin_transform_before_deepreg_COLIHM62_as_moving_mat_on_mask_ris_deepreg_then_to_snipr.sh $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input1 /output
fi
...
if [[ ${TYPE_OF_PROGRAM} == 'AFFINE_TRAN_BEFORE_NONLIN_RIS_SNIPR_DISTRIBUTION' ]]; then
...
if [[ ${TYPE_OF_PROGRAM} == 'PDF_FOR_EDEMA_BIOMARKER_N_CSF_COMPARTMENT_SNIPR_MOUNT' ]]; then
#  rm /EDEMABIOMARKEROUTPUT/*.*
#  /software/nwucalculation_session_level_allsteps_on_snipr_with_mount_drives.sh $SESSION_ID $XNAT_USER $XNAT_PASS $XNAT_HOST /input1 /output
  cp /workingoutput/*.* /EDEMABIOMARKEROUTPUT/

  /software/edema_biomarker_N_csf_compartments_vols_N_display_for_NON_SAH_mount_snipr_drives.sh ${SESSION_ID} $XNAT_USER $XNAT_PASS $XNAT_HOST
fi
...
if [[ ${TYPE_OF_PROGRAM} == 'DELETE_INTERMEDIATE_FILES' ]]; then
PROJECT_ID=${SESSION_ID}
