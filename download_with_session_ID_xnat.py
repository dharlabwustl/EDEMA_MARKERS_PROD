...
XNAT_HOST_URL= "https://snipr02.nrg.wustl.edu:8080" ##'https://snipr.wustl.edu' ##os.environ['XNAT_HOST']  #'http://snipr02.nrg.wustl.edu:8080' #'https://snipr02.nrg.wustl.edu' #'https://snipr.wustl.edu'
...
atexit.register(xnatSession.disconnect)


def _upload_file_to_xnat(uri, local_path):
    """Upload one file through XNATpy's authenticated HTTP session.

    The existing pipeline posts multipart form data to a resource ``files/``
    endpoint.  XNATpy owns authentication and session renewal; the underlying
    interface is used here to preserve that established upload request shape.
    """
    full_url = XNAT_HOST.rstrip("/") + "/" + uri.lstrip("/")
    with open(local_path, "rb") as file_handle:
        response = xnatSession.interface.post(
            full_url,
...
def downloadresourcefilewithuri_py(url,dir_to_save):
    response = xnatSession.get(url['URI'])
...
def uploadfile():
    ...
    for eachniftifile in allniftifiles:
        response = _upload_file_to_xnat(url, eachniftifile)
        print(response)
    # for eachniftifile in allniftifiles:
...
def uploadsinglefile_with_URI(url,file_name,resource_dirname):
    try:
        ...
        response = _upload_file_to_xnat(url, file_name)
...
def download_a_singlefile_with_URLROW(url,dir_to_save):
    ...
    # subprocess.call(command,shell=True)
    response = xnatSession.get(url.loc[0,"URI"]) #/data/projects/ICH/resources/179772/files/ICH_CTSESSIONS_202305170753.csv") #
...
def delete_a_file_with_URIString(url):
    try:
        response = xnatSession.delete(url)
...
def download_a_singlefile_with_URIString(url,filename,dir_to_save):
    ...
    # command="echo  " + url['URI'] + " >> " +  os.path.join(dir_to_save,"test.csv")
    # subprocess.call(command,shell=True)
    response = xnatSession.get(url) #/data/projects/ICH/resources/179772/files/ICH_CTSESSIONS_202305170753.csv") #
    #                                                       # "/data/experiments/SNIPR02_E03548/scans/1-CT1/resources/147851/files/ICH_0001_01022017_0414_1-CT1_threshold-1024.0_22121.0TOTAL_VersionDate-11302022_04_22_2023.csv") ## url['URI'])
...
def download_an_xmlfile_with_URIString(args): #url,filename,dir_to_save):
    ...
    try:
        ...
        subprocess.call("echo " + "I url AT ::{}  >> /workingoutput/error.txt".format(XNAT_HOST.rstrip('/') + url) ,shell=True )
        ...
        try:
            ...
            subprocess.call(
                "echo " + "XNATpy XML download failed: {} >> /workingoutput/error.txt".format(exc),
                shell=True,
            )
            raise
            # return num_files_present
