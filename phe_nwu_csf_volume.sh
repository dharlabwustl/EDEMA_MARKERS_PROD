...
from module_PHE_NWU_CSFCompartment_Calculations  import * ;    measure_compartments_with_reg_round5_one_file_sh_phe()"  $grayimage $betimage  $csfmaskimage ${infarctmaskimage}  ${npyfiledirectory}     ${output_directory}  ${lower_threshold} ${upper_threshold} ${session_id} #$gray_bet_extension $yashengfolder $infarct_mask_ext $lower_threshold $upper_threshold  ${levelsetfile_directory}  #$method_type  $method_type_name #$static_template_image $new_image $backslicenumber #$single_slice_filename

#from module_PHE_NWU_CSFCompartment_Calculations_v1  import * ;    measure_compartments_with_reg_round5_one_file_sh_phe()"  $grayimage $betimage  $csfmaskimage ${infarctmaskimage}  ${npyfiledirectory}     ${output_directory}  ${lower_threshold} ${upper_threshold} ${session_id} #$gray_bet_extension $yashengfolder $infarct_mask_ext $lower_threshold $upper_threshold  ${levelsetfile_directory}  #$method_type  $method_type_name #$static_template_image $new_image $backslicenumber #$single_slice_filename
