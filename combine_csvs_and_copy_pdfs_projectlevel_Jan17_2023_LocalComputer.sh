...
output_directory="/workingoutput"
for x in $(find $working_dir -name *columndropped.csv) ; do echo $(basename $x) ; cp $x ${working_dir}/  ; done
combine_all_csvfiles_of_edema_biomarker  ${working_dir} ${output_directory} ${extension_csv} ${combined_csv_outputfilename}
for x in $(find $working_dir -name *.pdf) ; do echo $(basename $x) ; cp $x ${final_output_directory}/  ; done
for x in $(find output_directory -name *COMBINED_columndropped.csv) ; do echo $(basename $x) ; cp $x ${final_output_directory}/  ; done
######################################################################################################################
