    ...
    if [ -f "${niftifile_csvfilename}" ]; then
      ...
      while IFS=',' read -ra array; do
        ...
        if [ ${NIFTIFILE_FLAG} -eq 1 ]; then
          ...
          URI=${array[0]}
          call_fill_single_row_each_scan_arguments=('call_fill_single_row_each_scan' ${output_csvfile%.nii*} "SESSION_ID" ${sessionID} ${final_output_directory}/csvfilename.csv)
