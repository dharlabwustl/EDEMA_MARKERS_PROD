...
def download_a_singlefile_with_URLROW(url,dir_to_save):
    ...
    zipfilename=os.path.join(dir_to_save,os.path.basename(url.loc[0,"URI"]) ) #"/data/projects/ICH/resources/179772/files/ICH_CTSESSIONS_202305170753.csv")) #sessionId+scanId+'.zip'
    with open(zipfilename, "wb") as f:
        for chunk in response.iter_content(chunk_size=512):
            if chunk:  # filter out keep-alive new chunks
                f.write(chunk)
