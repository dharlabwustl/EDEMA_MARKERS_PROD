...
def call_lobar_location_distribution(args): #SESSION_ID):
    ...
    try:
        ...
        SESSION_ID=args.stuff[1] ##str(row_item['ID'])
        print(SESSION_ID)
        return_value=lobar_region_volumes_n_display(SESSION_ID)
        subprocess.call("echo " + "I call_lobar_location_distribution return_value  ::{}  >> /workingoutput/error.txt".format(return_value) ,shell=True )
