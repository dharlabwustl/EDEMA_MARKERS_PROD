...
def fill_analytics_parallel(row):
    try:
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
    futures = {executor.submit(fill_analytics_parallel, row): row for _, row in copy_session_df.iterrows()}
...
def add_pdf_size_parallel(row):
    if '.pdf' in str(row['PDF_FILE_PATH']):
        ...
        counter=counter+1
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
    futures = {executor.submit(add_pdf_size_parallel, row): row for _, row in copy_session_df.iterrows()}
