...
while IFS=',' read -ra array; do
  ...
  while IFS=',' read -ra array1; do
    ...
    registration_each_scan ${filename_nifti}
    ...
    template_gray_filename=${final_output_directory}/${filename%.nii*}_resaved_levelset_brain_fscct_strippedResampled1lin1_1.nii.gz
    template_bet_maskfilename=${final_output_directory}/${filename%.nii*}_resaved_levelset_brain_fscct_strippedResampled1lin1_1_BET.nii.gz
    thisfile_bet_gray_filename=${final_output_directory}/${filename%.nii*}_resaved_levelset_brain_f.nii.gz
    thisfile_bet_mask_filename=${final_output_directory}/${filename%.nii*}_resaved_levelset_brain_f_BET.nii.gz
    #    URI_1=${url1%/resources*}
