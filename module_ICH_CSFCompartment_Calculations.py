...
def measure_compartments_with_reg_round5_one_file_sh_v1() : #niftifilenamedir,npyfiledirectory,npyfileextension):
    ...
    if gray_image_data.shape[0] == bet_image_data.shape[0] == csf_image_data.shape[0]  and gray_image_data.shape[1] == bet_image_data.shape[1] == csf_image_data.shape[1]  and  gray_image_data.shape[2] == bet_image_data.shape[2] == csf_image_data.shape[2]:
        ...
        row = ["FileName_slice" , "LEFT CSF VOLUME", "RIGHT CSF VOLUME","TOTAL CSF VOLUME", "ICH SIDE","ICH VOLUME","ICH EDEMA VOLUME", "BET VOLUME","CSF RATIO","LEFT BRAIN VOLUME without CSF" ,"RIGHT BRAIN VOLUME without CSF","CSFVOXEL_NUM_OVERLAP_EDEMA","CSFVOXEL_VOL_OVERLAP_EDEMA","LEFT CSF VOLUME AFTER EDEMA SUBT", "RIGHT CSF VOLUME AFTER EDEMA SUBT", "CSF RATIO AFTER EDEMA SUBT" ]
        ...
        if os.path.exists(CSF_Mask_filename) and os.path.exists(niftifilename): # and os.path.exists(ICH_mask_basename_path) :
            ...
            row2 = [thisfilebasename ,str(left_pixels_num), str(right_pixels_num),str(left_pixels_num+right_pixels_num), ICH_side,overall_ICH_vol,overall_ICH_vol_class2, str(BET_VOLUME),str(CSF_RATIO),str(left_brain_volume),str(right_brain_volume),EDEMA_VOXELS_IN_CSF,EDEMA_VOXELS_IN_CSF_TOTAL_VOL,left_pixels_num_after_edema_subt,right_pixels_num_after_edema_subt,CSF_RATIO_after_edema_subt]
