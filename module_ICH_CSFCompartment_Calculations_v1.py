...
def measure_ICH_Class1_Class2_Jan12_2024(): #niftifilename,npyfiledirectory,niftifilenamedir):
    ...
    ##############################################################################################################################
            ...
            if len(ICH_Mask_filename_data_np.shape) == 4:
                ICH_Mask_filename_data_np=ICH_Mask_filename_data_np[:,:,:,0]
    ###############################################################################################################
            if 'hdr' in PHE_Mask_filename_part2:
                PHE_Mask_filename_data_np=resizeinto_512by512(nib.AnalyzeImage.from_filename(PHE_Mask_filename).get_fdata()) #nib.load(PHE_Mask_filename).get_fdata()
                niftifilename_data=nib.load(niftifilename).get_fdata()
                if niftifilename_data.shape[2] > PHE_Mask_filename_data_np.shape[2]:
                    difference_slices=niftifilename_data.shape[2] - PHE_Mask_filename_data_np.shape[2]
                    for slice_num in range(difference_slices):
                        PHE_Mask_filename_data_np[:,:,PHE_Mask_filename_data_np.shape[2]+slice_num]=PHE_Mask_filename_data_np[:,:,0]
            #             filename_gray_data_np_copy=np.copy(niftifilename_data)

            if 'gz' in PHE_Mask_filename_part2:
                PHE_Mask_filename_data_np=resizeinto_512by512(nib.load(PHE_Mask_filename).get_fdata()) #resizeinto_512by512(image_nib_nii_file_data)
                ...
                ## volume of the PHE mask:
                PHE_total_voxels = PHE_Mask_filename_data_np[PHE_Mask_filename_data_np>np.min(PHE_Mask_filename_data_np)]
                PHE_total_voxels_count=PHE_total_voxels.shape[0]
                PHE_total_voxels_volume = PHE_total_voxels.shape[0]*np.prod(np.array(nib.load(niftifilename).header["pixdim"][1:4]))
                PHE_total_voxels_volume=PHE_total_voxels_volume/1000
            ...
            for img_idx in range(numpy_image.shape[2]):
                ...
                if img_idx>0 and img_idx < numpy_image.shape[2] and  filename_gray_data_np_copy.shape==csf_seg_np.shape:
                    ...
                    if os.path.exists(this_npyfile):
                        ...
                        # # I4[I_t_r_f_rinv_tinv_mask>0]=255
                        I4[np.copy(numpy_image_mask[:,:,img_idx])>0]=255
                        I4[np.copy(numpy_phe_image_mask[:,:,img_idx])>0]=255
                        # I5=np.copy(filename_gray_data_np_copy[:,:,img_idx])
                        cv2.imwrite(os.path.join(niftifilenamedir,"I4_img" +grayfilename_base + "_1.jpg"),I4)
                        I4_img_1=cv2.imread(os.path.join(niftifilenamedir,"I4_img" +grayfilename_base + "_1.jpg"))
                        img_with_line1=cv2.line(I4_img_1, (int(points1[0][0]),int(points1[0][1])), (int(points1[1][0]),int(points1[1][1])), (0,255,0), lineThickness)
                        ...
                        slice_3_layer1[:,:,0][np.copy(numpy_phe_image_mask[:,:,img_idx])>0]=50 #100
                        slice_3_layer1[:,:,1]= filename_gray_data_np_copy[:,:,img_idx] #imgray1
                        slice_3_layer1[:,:,1][np.copy(numpy_image_mask[:,:,img_idx])>0]=255 #0
                        slice_3_layer1[:,:,1][np.copy(numpy_phe_image_mask[:,:,img_idx])>0]=250 #100
                        slice_3_layer1[:,:,2]= filename_gray_data_np_copy[:,:,img_idx]# imgray1
                        slice_3_layer1[:,:,2][np.copy(numpy_image_mask[:,:,img_idx])>0]=255 #100
                        slice_3_layer1[:,:,2][np.copy(numpy_phe_image_mask[:,:,img_idx])>0]=100 #100
                        slice_3_layer1=cv2.line(slice_3_layer1, (int(points1[0][0]),int(points1[0][1])), (int(points1[1][0]),int(points1[1][1])), (0,255,0), lineThickness)
                        numpy_image_mask_flatten=numpy_image_mask[:,:,img_idx].flatten()
                        ICH_pixels_number=ICH_pixels_number+len(numpy_image_mask_flatten[np.nonzero(numpy_image_mask_flatten)]) #ICH_pixels.flatten()
                        cv2.imwrite(imagename,img_with_line1)
...
def csf_ratio_after_subtractionof_edema(niftifilename,bet_filename_path,grayfilename,ICH_Class2_Mask_filename_data,CSF_Mask_filename_data_np,npyfiledirectory,latexfilename,SLICE_OUTPUT_DIRECTORY):
    ...
    # filename_gray_data_np[ICH_Class2_Mask_filename_data_512>np.min(ICH_Class2_Mask_filename_data_512)]=np.min(filename_gray_data_np)
    # filename_gray_data_np_1[ICH_Class2_Mask_filename_data_512>np.min(ICH_Class2_Mask_filename_data_512)]=np.min(filename_gray_data_np_1)
    CSF_Mask_filename_data_np[ICH_Class2_Mask_filename_data_512>np.min(ICH_Class2_Mask_filename_data_512)]=np.min(CSF_Mask_filename_data_np)
    ...
    if gray_image_data.shape[0] == bet_image_data.shape[0] == csf_image_data.shape[0]  and gray_image_data.shape[1] == bet_image_data.shape[1] == csf_image_data.shape[1]  and  gray_image_data.shape[2] == bet_image_data.shape[2] == csf_image_data.shape[2]:
        ...
        if os.path.exists(sys.argv[4]):
            ...
            if  gray_image_data.shape[1]  ==  ICH_image_data.shape[1] and  gray_image_data.shape[0] == ICH_image_data.shape[0]    and gray_image_data.shape[2] == ICH_image_data.shape[2]:
                ...
                lower_thresh,upper_thresh,lower_thresh_normal,upper_thresh_normal, ICH_total_voxels_volume,ICH_side,NWU,ICH_pixels_number,ICH_pixels_density,nonfarct_pixels_number,nonICH_pixels_density, overall_ICH_vol,overall_non_ICH_vol=measure_ICH_Class1_Class2_Jan12_2024() # measure_ICH_Class1_Feb24_2023
        ...
        row = ["FileName_slice" , "LEFT CSF VOLUME", "RIGHT CSF VOLUME","TOTAL CSF VOLUME", "ICH SIDE","ICH VOLUME","ICH EDEMA VOLUME", "BET VOLUME","CSF RATIO","LEFT BRAIN VOLUME without CSF" ,"RIGHT BRAIN VOLUME without CSF","CSFVOXEL_NUM_OVERLAP_EDEMA","CSFVOXEL_VOL_OVERLAP_EDEMA", "RIGHT CSF VOLUME AFTER EDEMA SUBT","LEFT CSF VOLUME AFTER EDEMA SUBT", "CSF RATIO AFTER EDEMA SUBT" ]
        ...
        if os.path.exists(CSF_Mask_filename) and os.path.exists(niftifilename): # and os.path.exists(ICH_mask_basename_path) :
            ...
            left_pixels_num_after_edema_subt,right_pixels_num_after_edema_subt,CSF_RATIO_after_edema_subt=csf_ratio_after_subtractionof_edema(niftifilename,bet_filename_path,grayfilename,ICH_Class2_Mask_filename_data,CSF_Mask_filename_data_np,npyfiledirectory,latexfilename,SLICE_OUTPUT_DIRECTORY)
            row2 = [thisfilebasename ,str(left_pixels_num), str(right_pixels_num),str(left_pixels_num+right_pixels_num), ICH_side,overall_ICH_vol,overall_ICH_vol_class2, str(BET_VOLUME),str(CSF_RATIO),str(left_brain_volume),str(right_brain_volume),EDEMA_VOXELS_IN_CSF,EDEMA_VOXELS_IN_CSF_TOTAL_VOL,right_pixels_num_after_edema_subt,left_pixels_num_after_edema_subt,CSF_RATIO_after_edema_subt]
        ...
        #     values_in_table_df=values_in_table_df.reindex([3,4,5,2,1,0,7,6,8,9,10,11,12,13,14])
