...
for row_id, row in new_analytics_file_df.iterrows():
  ...
  for each_col in new_analytics_file_df_cols:
    if each_col != "subject":
      ...
      if "date_time" in each_col:
        field_value=field_value ##datetime.datetime.strptime(datetime.datetime.strptime(row[each_col], '%m/%d/%Y %H:%M').strftime('%m-%d-%Y %H:%M'),'%m-%d-%Y %H:%M')
      record = {
      ...
      try:
        ...
        print('FIELD NAME: ' + str(field_id+'::'+str(field_value)))
  ...
  if ".pdf" in str(PDF_FILE_PATH):
    ...
    download_a_singlefile_with_URIString(get_latest_filepath_from_metadata_arguments)
  if ".pdf" in PDF_FILE_PATH_LOCAL and os.path.exists(PDF_FILE_PATH_LOCAL):
...
# for row_id, row in copy_session_df.iterrows():
#   # for each_col in copy_session_df_cols:
#   #   if each_col != "subject":
