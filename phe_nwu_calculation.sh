...
run_IML_NWU_CSF_CALC() {
  ...
  /software/phe_nwu_csf_volume.sh ${this_filename} ${this_betfilename} ${this_csfmaskfilename} ${this_infarctmaskfilename} ${lower_threshold} ${upper_threshold} ${sessionID}
...
nwucalculation_each_scan() {
  ...
  for eachfile in ${working_dir_1}/*.nii*; do
    ...
    infarctfilename=${eachfile_basename_noext}_resaved_4DL_normalized_class2.nii.gz #_resaved_infarct_auto_removesmall.nii.gz
    ################################################
...
echo niftifile_csvfilename::$niftifile_csvfilename 
