...
def superimpose_singlemask_on_gray_ct_original(grayscale_img_path, infarct_mask_filename, output_dir, color_tuple, image_prefix,thresh=[20,60]):
    # Load and normalize grayscale image
    subprocess.call("echo " + "I PASSED AT TESTING superimpose_singlemask_on_gray_ct_original::{}  >> /workingoutput/error.txt".format((f"{thresh}::{image_prefix}::{color_tuple}::{output_dir}::{infarct_mask_filename}::{grayscale_img_path}")) ,shell=True )
    grayscale_img = nib.load(grayscale_img_path).get_fdata()
    min_int=thresh[0]
